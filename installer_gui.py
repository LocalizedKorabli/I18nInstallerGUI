...
locale_config = '''<locale_config>
    ...
    <lang_mapping>
        <lang acceptLang="ru" egs="ru" fonts="EU" full="english" languageBar="false" localeRfcName="ru" short="ru" />
    </lang_mapping>
</locale_config>
'''
...
tooltip_download_mods = 'Visit Mods Market'
...
class LocalizationInstallerAuto:
    ...
    def __init__(self, parent: tk.Tk, options: Any):
        ...
        self.install_progress_bar = ttk.Progressbar(parent, variable=self.install_progress, maximum=100.0,
                                                    style='success-striped', length=750)
...
class LocalizationInstaller:
    ...
    def __init__(self, parent: tk.Tk):
        ...
        self.auto_search = ttk.Button(parent, text='Auto Detect', command=lambda: self.find_game(overwrite=False),
                                      style='success')
        self.auto_search.grid(row=1, column=2, columnspan=1, padx=5, sticky='news')
        ...
        self.game_path_button.grid(row=1, column=3, columnspan=1, padx=5, sticky='news')
        ...
        self.choose_au_shortcut_path_button = ttk.Button(parent, text='Select Path',
                                                         command=self.choose_au_shortcut_path,
                                                         style='dark')
        ...
        self.src_button = ttk.Button(parent, text='Source Code',
                                     ...
                                     style=ttk.DANGER)
        self.src_button.grid(row=17, column=1, pady=5, padx=5, sticky='news')
    ...
    def on_download_source_changed(self, *args):
        if self.download_source.get() == 'local':
            ...
            self.download_progress_label.grid(row=9, column=0, padx=5, pady=5, columnspan=1, sticky=tk.W)
    ...
    def install_update(self):
        if self.is_installing:
            Messagebox.show_warning('Installation in progress!', 'Installation')
            return
...
def _install_update(
    ...
    for run_dir in run_dirs:
        ...
        if nothing_wrong:
            if full_gui:
                gui.safely_set_install_progress_text('Installing I18n Package - Moving')
...
def get_mods(mods_selection: bool, instance_dir: Path) -> List[str]:
    if not mods_selection:
        return []
    mods_dir = instance_dir.joinpath('i18n_installer').joinpath('mods')
...
def parse_and_apply_mods(gui: Union[LocalizationInstaller, LocalizationInstallerAuto], downloaded_mo: str,
    ...
    if full_gui:
        gui.safely_set_install_progress_text('Installing I18n Package - Applying Mods')
...
def run():
    ...
    if options.auto is False:
        ...
        half_screen_width = int(root.winfo_screenwidth() / 2) - 287
